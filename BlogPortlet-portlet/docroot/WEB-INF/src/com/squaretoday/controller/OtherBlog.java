...
/**
 * Portlet implementation class OtherBlog
 */
public class OtherBlog extends MVCPortlet {
		...
		public void doView(RenderRequest renderRequest,
			...
			try {
				long plid=LayoutLocalServiceUtil.getFriendlyURLLayout(themeDisplay.getLayout().getGroupId(), false, "/blogs").getPlid();
		...
		private Set<Integer> getArticleYears(ThemeDisplay themeDisplay)
		{
			...
			assetEntryQuery.setAnyCategoryIds(getCategoryId());			
		...
		private List<String> getArticleAbstract(ThemeDisplay themeDisplay,PortletRequest portletRequest ){
			...
			try {
	           ...
	           else
	           {
				...
				assetEntryQuery.setAnyCategoryIds(getCategoryId());
			    assetEntryList= AssetEntryLocalServiceUtil.getEntries(assetEntryQuery);
			    System.out.println("size of entries"+assetEntryList.size()+"category size"+getCategoryId().length);
		...
		private long[] getCategoryId() {
			...
			long catid =0;
			try {
				List<AssetCategory> assestcat = AssetCategoryLocalServiceUtil.getCategories();
				if(Validator.isNotNull(assestcat) && (assestcat.size()>0 && assestcat.size()<4)){
			long   	catid1 =	assestcat.get(0).getCategoryId();
				System.out.println(catid1);
			long   	catid2 =	assestcat.get(1).getCategoryId();
			System.out.println(catid2);
			long   	catid3 =	assestcat.get(2).getCategoryId();
			return new long[]{catid1,catid2,catid3};
				}
